...
export async function GET(req) {
  ...
  // Paginate through ALL active prices for this product
  let page = await stripe.prices.list({ product: PRODUCT, active: true, limit: 100 });
  while (true) {
    for (const price of page.data) {
      if (KEEP.has(price.id)) {
        ...
        archived.push({ id: price.id, currency: price.currency, amount: price.unit_amount });
      }
    }
    if (!page.has_more) break;
    page = await stripe.prices.list({ product: PRODUCT, active: true, limit: 100, starting_after: page.data[page.data.length - 1].id });
  }

  return Response.json({ archived, keptCount: kept.length });
}
